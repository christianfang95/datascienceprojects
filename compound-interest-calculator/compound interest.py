...
#Plot compound interest rate with monthly contributions
fig = go.Figure()
...
#Calculate compound interest with varying interest rates
...
for j in range(len(rates)):
    for i in range(1, t * 12 + 1):
        ...
        rates_res =  rates_res.append({'Rate': round(100 * rates[j], 2), 'Month': Month, 'Invested': Invested, 'Value': Value}, ignore_index = True)

#Plot compound interest with varying interest rates
...
fig.update_layout(title = 'Value of investment, by annual interest rate',
                   xaxis_title = 'Months',
                   yaxis_title = 'Amount',
                   legend_title_text = "Interest Rate in %",
                   font = dict(
                        family = "Helvetica",
                        size = 12, 
    ))
...
#Calculate compound interest with different amounts
...
#Plot compound interest with different amounts
